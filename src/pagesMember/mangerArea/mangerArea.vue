      ...
      <uni-list>
        <uni-transition mode-class="slide-left" :show="show" :duration="600">
          <uni-swipe-action-item :right-options="options" @click="deleteScence(item)" v-for="(item, index) in list"
            ...
            <navigator :url="`/pagesMember/newArea/newArea?id=${item.id}&name=${item.name}`" open-type="navigate"
              hover-class="navigator-hover">
              <uni-list-item :title="item.name" showArrow thumb-size="base" :border="false"
